import { CalendarDays, Star, MapPin, BadgeCheck, Bookmark } from "lucide-react";
...
export default function WorkerCard({ worker, horizontal = false, saved = false, onToggleSave }: Props) {
  ...
  if (horizontal) {
    return (
      <div
        className="bg-white rounded-2xl shadow-sm border border-border p-3 w-[160px] flex-shrink-0 cursor-pointer active:scale-[0.98] transition-transform"
      ...
      >
        <div className="flex flex-col items-center gap-2">
          ...
          <button
            onClick={(e) => { e.stopPropagation(); setLocation(`/book/${worker.id}`); }}
            className="w-full flex items-center justify-center gap-1 bg-primary text-primary-foreground rounded-lg py-1.5 text-[10px] font-semibold"
            data-testid={`btn-book-${worker.id}`}
          >
